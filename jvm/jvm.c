...
/* Java class-file UTF-8 constants are length-prefixed and are NOT NUL-terminated. */
...
typedef struct { uint16_t count; CP *cp; const uint8_t *method_data; uint16_t methods; } Class;
typedef struct { const uint8_t *code; uint32_t len; } Code;

static int parse_class(const uint8_t *d,size_t n,Class*c){
    if(n<10||d[0]!=0xca||d[1]!=0xfe||d[2]!=0xba||d[3]!=0xbe)return-1;
    const uint8_t*p=d+8;uint16_t count=r16(&p);c->count=count;
    c->cp=(CP*)calloc(count,sizeof(CP));if(!c->cp)return-2;
    for(uint16_t i=1;i<count;i++){
        if((size_t)(p-d)>=n){free(c->cp);return-3;}
        uint8_t t=*p++;c->cp[i].tag=t;
        switch(t){
            case 1:{uint16_t l=r16(&p);c->cp[i].a=l;c->cp[i].off=(uint32_t)(p-d);if((size_t)(p-d)+l>n){free(c->cp);return-3;}p+=l;break;}
            case 3:case 4:c->cp[i].a=r16(&p);c->cp[i].b=r16(&p);break;
            case 5:case 6:c->cp[i].a=r16(&p);c->cp[i].b=r16(&p);r16(&p);r16(&p);i++;break;
            case 7:case 8:case 16:case 19:case 20:c->cp[i].a=r16(&p);break;
            case 9:case 10:case 11:case 12:case 17:case 18:c->cp[i].a=r16(&p);c->cp[i].b=r16(&p);break;
            case 15:r16(&p);c->cp[i].b=r16(&p);break;
...
static int find_main(Class*c,const uint8_t*d,Code*out){
    ...
    for(uint16_t i=0;i<c->methods;i++){
        ...
        if(ni<c->count&&di<c->count&&c->cp[ni].tag==1&&c->cp[di].tag==1&&
           utf_eq(d,c->cp[ni].off,c->cp[ni].a,"main")&&
           utf_eq(d,c->cp[di].off,c->cp[di].a,"([Ljava/lang/String;)V")){
...
static int cp_utf_eq(const Class*c,const uint8_t*d,uint16_t i,const char*s){
    return i<c->count&&c->cp[i].tag==1&&utf_eq(d,c->cp[i].off,c->cp[i].a,s);
}

static int str_const(const Class*c,const uint8_t*d,uint16_t i,char*out,size_t cap){
...
static int native_call(const Class*c,const uint8_t*d,uint16_t mr,int argc,int32_t*args){
    ...
    uint16_t ci=c->cp[mr].a,nt=c->cp[mr].b;
    if(ci>=c->count||nt>=c->count||c->cp[ci].tag!=7||c->cp[nt].tag!=12)return-1;
...
int jvm_run(const uint8_t*d,size_t n){
    ...
    rc=find_main(&c,d,&code);if(rc){fprintf(stderr,"JVM: main([Ljava/lang/String;)V not found (%d)\n",rc);free(c.cp);return rc;}
