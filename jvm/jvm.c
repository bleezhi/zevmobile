...
static int parse_class(const uint8_t*d,size_t n,Class*c){
    ...
    const uint8_t*p=d+8,*end=d+n;uint16_t count=r16(&p);c->count=count;
    ...
    for(uint16_t i=1;i<count;i++){
        ...
        switch(t){
            ...
            case 5:case 6:if(!have(p,end,8)){free(c->cp);return-3;}r16(&p);r16(&p);r16(&p);r16(&p);i++;break;
...
int zev_jvm_run_main(ZevJvm*jvm){if(!jvm||!jvm->loaded_class.data){if(jvm)jvm->last_error=-1;return-1;}jvm->running=1;int rc=jvm_run(jvm->loaded_class.data,jvm->loaded_class.size);jvm->running=0;jvm->exit_code=rc;jvm->last_error=rc;return rc;}
